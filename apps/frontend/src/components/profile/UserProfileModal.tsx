    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-surface w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-[32px] shadow-2xl animate-in fade-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700 flex flex-col">
                ...
                {/* Header */}
                <div className="p-6 md:p-8 border-b border-border flex justify-between items-start bg-surface sticky top-0 z-10">
                    <h2 className="text-2xl font-bold text-foreground">상세 프로필</h2>
                    <button onClick={onClose} className="p-2 -mr-2 text-foreground-muted hover:text-foreground transition-colors rounded-full hover:bg-surface-hover">
                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
                                <div className="w-24 h-24 rounded-[2rem] bg-background ring-4 ring-surface shadow-lg overflow-hidden flex items-center justify-center text-4xl shrink-0">
                                ...
                                <div className="flex-1">
                                    <div className="flex flex-col md:flex-row items-center md:items-start gap-2 mb-2">
                                        ...
                                        {user.teacherProfile?.isVerified && (
                                            <span className="px-2 py-0.5 bg-blue-50 text-blue-600 dark:bg-blue-900/40 dark:text-blue-300 text-[10px] font-bold rounded-md border border-blue-100 dark:border-blue-800">신원인증됨</span>
                                        )}
                                        {user.businessProfile?.s2bNumber && (
                                            <span className="px-2 py-0.5 bg-purple-50 text-purple-600 dark:bg-purple-900/40 dark:text-purple-300 text-[10px] font-bold rounded-md border border-purple-100 dark:border-purple-800">S2B 등록업체</span>
                                        )}
                                    </div>

                                    <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                                        {user.role === Role.TEACHER && user.teacherProfile?.subjects.map(s => (
                                            <span key={s} className="px-3 py-1 bg-surface-hover text-foreground-muted rounded-lg text-xs font-bold ring-1 ring-inset ring-slate-200/50 dark:ring-slate-700">{s}</span>
                                        ))}
                                        {user.role === Role.BUSINESS && user.businessProfile?.categories.map(c => (
                                            <span key={c} className="px-3 py-1 bg-surface-hover text-foreground-muted rounded-lg text-xs font-bold ring-1 ring-inset ring-slate-200/50 dark:ring-slate-700">{c}</span>
                                        ))}
                                    </div>

                                    {/* Bio / Description */}
                                    <div className="bg-background p-4 rounded-2xl border border-border text-sm text-foreground-muted leading-relaxed whitespace-pre-wrap text-left">
                ...
                {/* Footer */}
                <div className="p-4 md:p-6 border-t border-border bg-surface-hover flex justify-end">
                    <button
                        onClick={onClose}
