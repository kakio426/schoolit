...
export default function JobsPage() {
    ...
    const handleDelete = async (jobId: number) => {
        if (!confirm('정말로 이 공고를 삭제하시겠습니까? 지원자 내역도 함께 삭제될 수 있습니다.')) return;
        try {
                                ...
                                <div key={job.id} className="bg-surface p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className="px-3 py-2 rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors"
                                            title="삭제"
                                        >
                                            🗑️
                                        </button>
    ...
    return (
        <DashboardLayout>
            <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {viewMode === 'MY' ? (
                    /* My Jobs View for Teachers (Similar to School View) */
                    <div className="space-y-4">
                        {jobs.length === 0 ? (
                            <div className="text-center py-20 text-foreground-muted">등록한 행사 요청이 없습니다.</div>
                        ) : (
                            jobs.map((job) => (
                                <div key={job.id} className="bg-surface p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div>
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => toggleStatus(job.id, job.active)}
                                            className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${job.active ? 'border-red-200 text-red-500' : 'border-green-200 text-green-500'}`}
                                        >
                                            {job.active ? '마감하기' : '다시 열기'}
                                        </button>
                                        <Link
                        ...
                        <div className="flex justify-center mt-8">
                            <Link href="/dashboard/jobs/new" className="px-6 py-3 bg-primary text-white rounded-xl font-bold shadow-lg hover:bg-primary/90">
                                + 새 행사 요청 등록
                            </Link>
                        </div>
                    </div>
