            ...
            <div className="max-w-6xl mx-auto space-y-6 pb-20">
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-20 bg-slate-800/30 border border-slate-700 rounded-xl">
                        ...
                        <h3 className="text-white font-medium mb-1">공고가 없습니다</h3>
                    ...
                    <div className="space-y-3">
                        {jobs.map((job) => (
                            <Link
                                ...
                                className="group block bg-slate-800/40 border border-slate-700 hover:border-blue-600/50 hover:bg-slate-800/60 rounded-xl transition-all overflow-hidden"
                            >
                                <div className="flex flex-col md:flex-row">
                                    ...
                                    <div className="md:w-48 p-5 border-t md:border-t-0 md:border-l border-slate-700/50 flex flex-row md:flex-col items-center justify-between md:justify-center gap-3 bg-slate-800/20">
                                        {/* Status Badge */}
                                        {job.active ? (
                                            isDeadlineApproaching(job.createdAt) ? (
                                                <span className="flex items-center gap-1.5 px-2.5 py-1 bg-red-500/20 text-red-400 border border-red-500/30 rounded-full text-[10px] font-semibold">
                                                    <AlertCircle className="w-3 h-3" />
                                                    마감 임박
                                                </span>
                                            ) : (
                                                <span className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-full text-[10px] font-semibold">
                                                    <CheckCircle2 className="w-3 h-3" />
                                                    모집 중
                                                </span>
                                            )
                                        ) : (
                                            <span className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-600/50 text-slate-400 border border-slate-600 rounded-full text-[10px] font-semibold">
                                                모집 종료
                                            </span>
                                        )}
                                        ...
                                        {isSchool ? (
                                            <div className="flex items-center gap-2" onClick={(e) => e.preventDefault()}>
                                                ...
                                                <button
                                                    ...
                                                    className="p-1.5 text-slate-500 hover:text-red-400 border border-slate-700 hover:border-red-600/50 rounded-lg transition-all"
                                                >
