...
export class GamificationService {
  ...
  async calculateProfileCompleteness(
    ...
    else missing.push('이름');
    if (user.email) score += 10;
    else missing.push('이메일');

    // 2. Role Specifics (80%)
    if (user.role === Role.TEACHER && user.teacherProfile) {
      const p = user.teacherProfile;
      if (p.profileImage) score += 20;
      else missing.push('프로필 사진');
      if (p.bio) score += 20;
      else missing.push('자기소개');
      if (p.subjects && p.subjects.length > 0) score += 10;
      else missing.push('담당 과목');
      if (p.regions && p.regions.length > 0) score += 10;
      else missing.push('활동 지역');
      ...
      if (p.logoImage) score += 20;
      else missing.push('학교 로고');
      if (p.description) score += 20;
      else missing.push('학교 소개');
      if (p.address) score += 20;
      else missing.push('주소');
      if (p.phoneNumber) score += 20;
      else missing.push('대표 전화번호');
    } else if (user.role === Role.BUSINESS && user.businessProfile) {
      const p = user.businessProfile;
      if (p.description) score += 20;
      else missing.push('가게/기업 소개');
      if (p.address) score += 20;
      else missing.push('주소');
      if (p.categories && p.categories.length > 0) score += 20;
      else missing.push('카테고리 설정');
      if (p.portfolios && p.portfolios.length > 0) score += 20;
      else missing.push('포트폴리오 등록');
    }
  ...
  async updateTrustTier(userId: number) {
    ...
    if (currentTier === TrustTier.TRUSTED) {
      ...
      const avgRating =
        user.reviewsReceived.reduce((acc, r) => acc + (r.rating || 0), 0) / (reviewCount || 1);
