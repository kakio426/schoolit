...
export class ComplianceService {
  ...
  validateSalaryStep(
    salaryStep: number,
    isHonoraryRetiree: boolean = false,
  ): {
    isValid: boolean;
    message: string;
  } {
  ...
  async updateWorkflowStatus(
    ...
    if (
      newStatus === HiringWorkflowStatus.PUBLISHED ||
      newStatus === HiringWorkflowStatus.RECEIVING
    ) {
      data.status = 'OPEN'; // Using string literal or ensure JobStatus is imported
    } else if (
      newStatus === HiringWorkflowStatus.CANCELLED ||
      newStatus === HiringWorkflowStatus.DRAFT
    ) {
      data.status = 'CLOSED';
    }
